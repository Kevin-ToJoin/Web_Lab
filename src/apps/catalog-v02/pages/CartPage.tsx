...
### Bug Hints (6 bugs in this area):
- 🐛 **Input validation (Level 3):** Click the **−** button when quantity is 1. Can the quantity go to 0 or negative? What happens to the cart total?
- 🐛 **Data integrity (Level 7):** Add **PROD-001** (Ultra HD 4K Smart TV) to the cart, then add another product, then try to remove PROD-001. Which item actually disappears?
- 🐛 **Functional correctness (Level 8, Regex):** The valid promo codes are \`SAVE20\` and \`FALL10\`. Try a fake code that ends in "20" (e.g., \`HACK20\`, \`AAA20\`). Does it get accepted?
- 🐛 **Functional correctness (Level 8, Decision table):** Apply a promo, then check the order summary. Is the 8% tax applied on the **discounted** amount or the **full** subtotal? Check the DB for the expected formula.
- 🐛 **Security (Level 9):** Open your browser console and run \`localStorage.setItem('isAdmin', 'true')\`, then refresh the cart. What happens to the total?
- 🐛 **Usability (Level 2):** Type a clearly invalid promo code and apply it. Do you see any error message at all?
